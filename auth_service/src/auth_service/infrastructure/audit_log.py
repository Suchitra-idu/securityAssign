...
class PostgresAuditLog:
    ...
    def record(self, event: dict) -> None:
        # Wrap in an explicit txn so LOCK TABLE holds across the read+insert,
        # and so audit writes commit independently of the caller's txn (letting
        # failed-login events persist even when the request rolls back).
