...
def unfreeze_account(*, account_id: str, caller: Caller, deps: BankingDeps) -> Account:
    ...
    if account.status == "active":
        return account
    active = replace(account, status="active")
    deps.accounts.update(active)
    emit(deps, "account_unfrozen", account_id=account.id, actor_user_id=caller.user_id)
    return active
